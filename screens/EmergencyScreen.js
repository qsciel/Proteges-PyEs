...
export default function EmergencyScreen({ navigation }) {
  ...
  const toggleEmergency = async () => {
    if (emergencyActive) {
      ...
      Alert.alert(
        "Confirmar",
        "¿Está seguro de que desea ACTIVAR el protocolo de emergencia?",
        [
          { text: "Cancelar", style: "cancel" },
          {
            text: "ACTIVAR",
            style: "destructive",
            onPress: confirmTriggerEmergency,
          },
        ],
      );
    }
  ...
  const confirmTriggerEmergency = async () => {
    try {
      console.log("Iniciando emergencia para usuario:", user.id);
      const result = await api.triggerEmergency(true, user.id);
      console.log("Respuesta del servidor:", result);
      setEmergencyActive(true);
      fetchData();
      Alert.alert("Éxito", "Protocolo de emergencia activado correctamente");
    } catch (e) {
      console.error("Error al iniciar emergencia:", e);
      Alert.alert(
        "Error",
        `Fallo al iniciar emergencia: ${e.message || e.toString()}`,
      );
    }
  };
  ...
  const confirmEndEmergency = async () => {
    try {
      console.log("Finalizando emergencia para usuario:", user.id);
      const result = await api.triggerEmergency(false, user.id);
      console.log("Respuesta del servidor:", result);
      ...
      Alert.alert(
        "Finalizada",
        "Protocolo de emergencia desactivado correctamente",
      );
    } catch (e) {
      console.error("Error al finalizar emergencia:", e);
      Alert.alert(
        "Error",
        `Fallo al finalizar emergencia: ${e.message || e.toString()}`,
      );
    }
  };
        ...
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert("Error", "Fallo al actualizar estado");
            }
          },
        },
      ],
            ...
            children: [
              ...
              new Paragraph({
                children: [
                  new TextRun({
                    text:
                      reportOptions.groupBy === "all"
                        ? "TODOS LOS ESTUDIANTES"
                        : reportOptions.groupBy === "missing"
                          ? "ESTUDIANTES FALTANTES"
                          : "ESTUDIANTES SEGUROS",
                    bold: true,
                    size: 28,
                    color:
                      reportOptions.groupBy === "safe" ? "059669" : "DC2626",
                  }),
                ],
                spacing: { after: 200 },
              }),
                      ...
                      new TableRow({
                        children: [
                          ...
                          new TableCell({
                            children: [
                              new Paragraph(
                                `${s.names} ${s.paternal_last_name} ${s.maternal_last_name || ""}`,
                              ),
                            ],
                          }),
                          new TableCell({ children: [new Paragraph(s.group)] }),
                          new TableCell({
                            children: [new Paragraph(s.major || "N/A")],
                          }),
                          ...(reportOptions.includeContactInfo
                            ? [
                                new TableCell({
                                  children: [new Paragraph("Ver sistema")],
                                }),
                              ]
                            : []),
                        ],
    ...
    >
      ...
      <View style={styles.progressContainer}>
        <View
          style={[
            styles.progressBar,
            { flex: item.safe, backgroundColor: COLORS.success },
          ]}
        />
        <View
          style={[
            styles.progressBar,
            { flex: item.missing, backgroundColor: COLORS.danger },
          ]}
        />
      </View>
      <View style={styles.groupStats}>
        <Text style={{ color: COLORS.success, ...FONTS.bold }}>
          {item.safe} Seguros
        </Text>
        <Text style={{ color: COLORS.danger, ...FONTS.bold }}>
          {item.missing} Faltantes
        </Text>
      </View>
    </TouchableOpacity>
  );
  ...
  const renderStudentItem = (student) => (
    <TouchableOpacity
      ...
      style={[
        ...
        student.scanned
          ? {
              borderLeftColor: student.teacher_color || COLORS.success,
              borderLeftWidth: 4,
            }
          : { borderLeftColor: COLORS.danger, borderLeftWidth: 4 },
      ]}
    >
            ...
            <View>
              <View
                style={[
                  styles.badge,
                  {
                    backgroundColor: `${student.teacher_color || COLORS.success}15`,
                    borderColor: student.teacher_color || COLORS.success,
                  },
                ]}
              >
                <MaterialCommunityIcons
                  name="shield-check"
                  size={16}
                  color={student.teacher_color || COLORS.success}
                  style={{ marginRight: 4 }}
                />
                <Text
                  style={[
  ...
  return (
    <ScreenWrapper>
      <StatusBar
        ...
        backgroundColor={emergencyActive ? COLORS.danger : COLORS.primary}
      />
      <View
        style={[
          styles.header,
      ...
      >
        ...
        >
          <MaterialCommunityIcons
            name={emergencyActive ? "alert-octagon" : "shield-check"}
            size={24}
            color="white"
            style={{ marginRight: 8 }}
          />
          <Text style={styles.headerSubtitle}>
            {emergencyActive ? "EMERGENCIA ACTIVA" : "SISTEMA NORMAL"}
          </Text>
        </View>
      </View>
      ...
      {emergencyActive ? (
        ...
        <View style={styles.safeStateContainer}>
          ...
          <Text style={styles.safeStateSubText}>
            Los estudiantes están seguros. Active solo en caso de emergencia
            real o simulacro.
          </Text>
        </View>
      )}
              ...
              renderItem={({ item }) => (
                <View style={styles.historyItem}>
                  ...
                  >
                    <MaterialCommunityIcons
                      name={
                        item.action === "REVOKED" ? "alert-circle" : "check"
                      }
                      size={20}
                      color={
                        item.action === "REVOKED"
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalSubtitle}>
              {selectedGroup?.safe} Seguros • {selectedGroup?.missing} Faltantes
            </Text>
      ...
      {["Director", "Operador"].includes(user?.role) && (
        <View style={styles.bottomControls}>
          <Button
            title={
              emergencyActive ? "FINALIZAR EMERGENCIA" : "ACTIVAR EMERGENCIA"
            }
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalSubtitle}>
              Opciones para el reporte Word editable
            </Text>
            ...
            <ScrollView>
              <View style={{ paddingVertical: SPACING.m }}>
                ...
                >
                  <View style={{ flex: 1 }}>
                    <Text style={{ ...FONTS.semiBold, marginBottom: 4 }}>
                      Incluir Contactos
                    </Text>
                ...
                <View style={{ marginTop: SPACING.l }}>
                  <Text style={{ ...FONTS.semiBold, marginBottom: SPACING.m }}>
                    Mostrar Estudiantes:
                  </Text>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setReportOptions({ ...reportOptions, groupBy: "missing" })
                    }
                  >
                    <MaterialCommunityIcons
                      name={
                    ...
                    />
                    <Text style={{ marginLeft: SPACING.s }}>
                      {" "}
                      Solo Faltantes ({missingCount})
                    </Text>
                  </TouchableOpacity>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setReportOptions({ ...reportOptions, groupBy: "all" })
                    }
                  >
                    <MaterialCommunityIcons
                      name={
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setReportOptions({ ...reportOptions, groupBy: "safe" })
                    }
                  >
                    <MaterialCommunityIcons
                      name={
                        reportOptions.groupBy === "safe"
                          ? "radiobox-marked"
                          : "radiobox-blank"
                      }
                      size={24}
...
const styles = StyleSheet.create({
  ...
  groupIcon: {
    ...
    height: 40,
    borderRadius: 20,
    backgroundColor: COLORS.surfaceWithOpacity,
    alignItems: "center",
    justifyContent: "center",
